  ...
  return (
    <>
      ...
      <div className='grid_'>
        ...
        <BigButton text='体温记录' src={TemperaturePNG} onClick={() => Taro.navigateTo({ url: '/pages/temper/index' })} />
      </div>
    ...
    <View className='noticeview'>
      ...
      <ScrollView scrollY scrollTop={0} lowerThreshold={20} upperThreshold={20}>
        {debugList.map((item, index) => {
          return (
            <>
              <Cell key={item.time || index} className='gap-x-4 py-5 noticeview-item' radius='20px'>
                <View className='flex flex-col min-w-0'>
                  <Text className='text-sm font-semibold leading-6 text-gray-900'>{item.title}</Text>
                  <Text className='mt-1 truncate text-xs leading-5 text-gray-500'>{item.content}</Text>
                </View>
              </Cell>
            </>
          )
