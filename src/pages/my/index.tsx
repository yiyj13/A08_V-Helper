...
function ProfileCard() {
  ...
  return (
    <div
      className='flex p-6 items-center justify-between active:bg-gray-100 active:scale-105 transition-all'
      onClick={handleClick}
    >
      <div className='flex flex-row items-center'>
        ...
        <div>
          <h4 className='text-lg font-bold'>{data?.userName || '点击编辑用户信息'}</h4>
          <p className='mt-1 text-gray-500 font-mono'>{`id: ${getUserID()}`}</p>
        </div>
      </div>
      <RectRight />
    </div>
...
function ActionFlexColItem(props: IActionItem) {
  ...
  return (
    <div
      className='flex flex-row bg-white px-8 py-4 justify-between items-center active:bg-gray-100 transition-all active:scale-105'
      onClick={onClick}
    >
      <div className='flex flex-row items-center'>
        {icon}
        <p className={clsx('ml-4 text-sm font-semibold', textClass)}>{text}</p>
      </div>
